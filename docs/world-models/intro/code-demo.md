...
用一个列表把六条技术路径的简称、全称、代表方法、颜色统一管理，后面绘制分类地图和雷达图都直接遍历这个数据结构——把"数据"和"绘图逻辑"分开，方便后续增删路径。
...
以"World Model"为根节点，向右延伸出六条分支（每条技术路径一个节点），再向右延伸出代表方法（叶子节点）。`connectionstyle="arc3,rad=..."` 让连接线根据纵向偏移量自动弯曲成弧线，视觉上更像一张"思维导图"，而不是死板的直线连接。
...
| 概念 | 一句话解释 | 代码位置 |
|------|-----------|---------|
| 分类地图 | 六条技术路径的树状可视化 | `plot_taxonomy_map()` |
