...
> 前三章已经把 [GAN](/world-models/video/gan/)、[VAE](/world-models/video/vae/)、[扩散 / DiT](/world-models/video/diffusion/) 讲完。当视频模型能在较长时间内保持物体身份、大致遵守物理、并接受文本或动作条件时，人们开始把它当作**世界模拟器**。这是路径一最「好看」、也往往最「贵」的一站。
...
常见两段式（前两段正是 [VAE](/world-models/video/vae/) 与 [扩散](/world-models/video/diffusion/)）：
...
- 要**数据引擎 / 开放视觉先验** → 路径一；
- 要**可玩、可漫游、显式相机接口** → [路径二](/world-models/interactive/overview/)；
- 要**样本高效控制、实时规划** → [路径三](/world-models/abstract/overview/)；
- 要**谓词与规则** → [路径五](/world-models/symbolic/overview/)。
...
视频世界模型把模拟推到媒体生成尺度；好看不等于可控，可控不等于可干预。路径一到此收束。下一站进入 [路径二导论](/world-models/interactive/overview/)，或直接读 [Genie](/world-models/interactive/genie/)。

## 📥 Code

| File | View | Download |
|------|------|----------|
| demo.py | [Open](./code-demo) | <a href="/notebook/code/world-models/video/sora/demo.py" target="_blank" download>Download</a> |
| exercise.py | [Open](./code-exercise) | <a href="/notebook/code/world-models/video/sora/exercise.py" target="_blank" download>Download</a> |
