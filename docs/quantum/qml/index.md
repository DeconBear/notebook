...
> 量子机器学习（QML）在 NISQ 上几乎总是**混合**的：经典网络整理特征，量子线路吃角度、吐测量，再接一个小分类头。本章把这条管线讲清楚，并收编 [qml-mnist-classify](https://github.com/DeconBear/qml-mnist-classify)（MIT）里基于本源 **VQNet** 的 8 比特实验。
...
> **图解说明**：与原仓 tutorials 一致——FC 压缩 → 两个角度头 → Ry/Rz 写入 8 比特 → 固定 HEA → MeasureAll → 分类。
...
原仓库 [tutorials/02](https://github.com/DeconBear/qml-mnist-classify/blob/master/tutorials/02_classical_to_quantum_interface.md) 把接口拆成四步，本章直接采用：
...
完整训练入口仍是迁入的 `train.py` / `eval.py` / `qml_core.py`，需要本源 VQNet（[产品页](https://qcloud.originqc.com.cn/zh/programming/VQNet)）。`demo.py` 在能 `import pyvqnet` 时做一次前向冒烟；否则打印安装说明并**继续跑经典基线**。
...
因此本章的对照实验是**公平任务上的工程对比**，不是「量子击败 LeNet」的榜单：

| | 量子混合（原仓 `core/`） | 本章 demo 经典基线 |
|--|--|--|
| 框架 | VQNet | PyTorch |
| 输入 | 16×16，数字 3 vs 6 | 同一套 npz |
| 量子资源 | 8 比特 + 固定 HEA | 无 |
| 默认依赖 | 需自备 `pyvqnet` | notebook 已有 torch |

原仓另有 28×28 的 LeNet-5 目录，含大量 PNG，**没有**迁进 notebook。若要对照原版 LeNet，请直接打开 [qml-mnist-classify](https://github.com/DeconBear/qml-mnist-classify)。

![经典小 CNN 基线](./images/qml_classical_baseline.png)
...
docs/quantum/qml/code/
  demo.py          # 经典 CNN + 可选 VQNet 冒烟
  exercise.py      # 角度头（纯 NumPy）
  qml_core.py      # 原仓共享核心（import 需要 pyvqnet）
  train.py / eval.py
  dataset/*.npz    # 1000+200 张 16×16
  LICENSE-qml-mnist-classify.txt
```

建议阅读顺序与原仓 tutorials 对齐：接口 → 训练循环 → 再决定是否安装 VQNet 跑满 24 epoch。
...
1. 原仓库：[DeconBear/qml-mnist-classify](https://github.com/DeconBear/qml-mnist-classify)（MIT）。
2. 本源 VQNet：[产品介绍](https://qcloud.originqc.com.cn/zh/programming/VQNet)、[pyQPanda3 量子 ML 接口](https://qcloud.originqc.com.cn/document/vqnet_api_cn/rst/qnn_pq3.html)。
3. Cerezo et al., *Variational quantum algorithms*. Nat. Rev. Phys. (2021)；McClean et al., barren plateaus (2018).
4. Mitarai et al. / Schuld et al.：参数移位与混合量子-经典求导。
