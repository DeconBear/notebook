...
> s18 是 NLP 路线（s14-s18）的终点。从这里出发，你可以探索 s22（多模态）、s23（RAG 与 Agent）和 s24（部署与推理优化）。若关心「参数很大、每步只激活一部分」的稀疏大模型，见深度学习扩展章 [混合专家 MoE](/nn-decision/dl/moe/)。

## 📥 Code

| File | View | Download |
|------|------|----------|
| demo.py | [Open](./code-demo) | <a href="/notebook/code/applied/nlp/llm/demo.py" target="_blank" download>Download</a> |
| exercise.py | [Open](./code-exercise) | <a href="/notebook/code/applied/nlp/llm/exercise.py" target="_blank" download>Download</a> |
